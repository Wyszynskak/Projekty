#'
#' Liczenie odleglosci dla aktorow
#'
#' Funkcja \code{dAktorzy} liczy odleglosc miedzy aktorami dwoch filmow z bazy
#'
#' @param IDFilm1 - id pierwszego filmu
#' @param IDFilm2 - id drugiego filmu
#' @param sciezkaDoBazy - sciezka do bazy filmow
#'
#'@details Funkcja wyciaga ID dwoch filmow z bazy. Jesli ktorys z nich nie ma aktorow odleglosc
...
#' dAktorzy(4000,2461,"BazaFilmow.sql")
#' dAktorzy(23456,2461,"BazaFilmow.sql")
#'
#'@import RSQLite, stringi
#'


dAktorzy<-function(IDFilm1,IDFilm2,sciezkaDoBazy)
{
   baza_filmow<-dbConnect(SQLite(),dbname=sciezkaDoBazy)

   #Tworzenie komend w SQL
   komenda1<-paste0("select Imie,Nazwisko from Aktorzy where IDFilm=",IDFilm1)
   komenda2<-paste0("select Imie,Nazwisko from Aktorzy where IDFilm=",IDFilm2)


   #Wyciagam imiona i nazwiska aktorow pierwszego
   nazwa1 <- dbGetQuery(baza_filmow,komenda1)
   #Wyciagam imiona i nazwiska aktorow drugiego
